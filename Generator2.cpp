...
int myCounter = 0;  //perhaps i should use a pointer to avoid the global variable
...
void NewNodeLocationsArray (TreeNode *root, int TheArray[]) {     //This seems like a better way to do things, but it causes a seg fault...
	myCounter=0;
	TreeNode *tempnode;
	NodeLocation temptreequeue;
	temptreequeue.TheNode=root;
	temptreequeue.depth=0;
	for (int i=0; i<MAXDEPTH; i++) {
		temptreequeue.location[i]=-1;
	}
	queue<NodeLocation> treequeue;
	treequeue.push(temptreequeue);
	while (treequeue.size()>0) {
		temptreequeue=treequeue.front();
		treequeue.pop();
		tempnode=temptreequeue.TheNode;
		for (int i=0; i<MAXDEPTH; i++) {
			TheArray[myCounter]=temptreequeue.location[i];
			myCounter++;
		}
		temptreequeue.depth++;
		for (int i=0; i<MAXCHILDREN; i++) {
			if (tempnode->children[i] !=NULL) {
				temptreequeue.location[temptreequeue.depth-1]=i;
				temptreequeue.TheNode=tempnode->children[i];
				treequeue.push(temptreequeue);
			}
		}
	}
...
void NodeLocationsArray2 (TreeNode *root, int TheArray[], int depth, int ParentLocation[MAXDEPTH], int ParentDirection, int targetdepth) {
	if (root != NULL) {
		...
		if (depth < targetdepth) {
			for (int i=0; i<MAXCHILDREN; i++) {
				NodeLocationsArray2(root->children[i],TheArray,depth+1,ThisLocation,i,targetdepth);
			}
		}
	}
}

void NodeLocationsArray (TreeNode *root, int TheArray[]) {   //A temporary fix for the order issue
	myCounter=0;
	for (int i=0; i<MAXDEPTH; i++) {
		NodeLocationsArray2(root,TheArray,0,NULL,NULL,i);
	}
}
...
int main()
{
	...
	myCounter = 0;
	NodeLocationsArray(mytree,TempNodeLocations);
